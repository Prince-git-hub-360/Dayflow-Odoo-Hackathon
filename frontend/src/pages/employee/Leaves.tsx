    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
            <FileText className="w-6 h-6 text-indigo-400" /> Leave Management
          </h1>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="p-6 border-b border-slate-800 flex items-center justify-between">
          <h3 className="font-bold text-slate-100 text-base">My Leave History</h3>
          <span className="text-xs text-slate-400">Total Applications: {leaves?.length || 0}</span>
        </div>
                ...
                {leaves?.map((leave) => (
                  <tr key={leave.id} className="hover:bg-slate-900/40 transition-colors">
                    ...
                    <td className="py-4 px-6 text-xs text-slate-400">
                      {leave.admin_comment ? (
                        <span className="text-indigo-300 italic">"{leave.admin_comment}"</span>
                      ) : (
                        '-'
                      )}
                    </td>
      ...
      >
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            <select
              {...register('leave_type')}
              className="w-full bg-slate-900 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
            >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              />
              {errors.start_date && (
                <p className="mt-1 text-xs text-rose-400">{errors.start_date.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.end_date && (
                <p className="mt-1 text-xs text-rose-400">{errors.end_date.message}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            />
            {errors.remarks && (
              <p className="mt-1 text-xs text-rose-400">{errors.remarks.message}</p>
            )}
          </div>
