...
export const Sidebar: React.FC = () => {
  ...
  const employeeLinks = [
    ...
    { to: '/payroll', label: 'Payroll & Payslip', icon: DollarSign },
  ...
  const adminLinks = [
    ...
    { to: '/admin/payroll', label: 'Payroll Management', icon: DollarSign },
  ...
  return (
    <aside className="w-64 glass-card border-r border-slate-200 dark:border-slate-800/80 min-h-screen flex flex-col justify-between p-4 sticky top-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-xl z-40">
      <div className="space-y-6">
        ...
        <div className="p-3 rounded-xl bg-slate-100 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 flex items-center gap-3">
          <div className="w-9 h-9 rounded-full bg-indigo-600 text-white font-bold flex items-center justify-center text-sm shrink-0">
            {user?.employee_profile?.first_name?.[0] || user?.email?.[0].toUpperCase()}
          </div>
      ...
      <div className="pt-4 border-t border-slate-200 dark:border-slate-800/80 space-y-2">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors"
        >
